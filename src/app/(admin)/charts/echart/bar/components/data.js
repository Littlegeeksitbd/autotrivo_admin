...
export const getProgressBarChart = () => ({
  tooltip: {
    trigger: 'axis',
    padding: [5, 0],
    backgroundColor: getColor('bg-card'),
...
export const getNegativeBarChart = () => ({
  tooltip: {
    trigger: 'axis',
    padding: [5, 0],
    backgroundColor: getColor('bg-card'),
...
export const getBarChartWithSeries = () => ({
  tooltip: {
    trigger: 'axis',
    padding: [5, 0],
    backgroundColor: getColor('secondary-bg'),
...
for (let i = 0; i < series.length; ++i) {
  ...
  for (let j = 0; j < series[i].data.length; ++j) {
    ...
    let value
    if (typeof data[j] === 'object' && data[j] !== null && !Array.isArray(data[j]) && 'value' in data[j]) {
      value = data[j].value
    } else {
      value = data[j]
    }
...
export const getHorizontalStackedBarChar = () => ({
  xAxis: {
    type: 'value',
    axisLine: {
      lineStyle: {
...
export function setBarRaceChartInstance(instance) {
...
const generateRandomData = () => {
  ...
  years.forEach((year) => {
    data[year] = Array.from(
      {
        length: 12,
      },
      () => +(Math.random() * 3000 + 100).toFixed(2)
    )
  })
  return data
}
...
export const getTimelineBarChart = () => ({
  baseOption: {
    timeline: {
      axisType: 'category',
      autoPlay: false,
  ...
  options: [2019, 2020, 2021, 2022, 2023, 2024, 2025].map((year) => ({
    ...
    series: [
      {
        data: chartData.dataPI[year],
      },
      {
        data: chartData.dataSI[year],
      },
      {
        data: chartData.dataTI[year],
      },
    ],
  })),
})
