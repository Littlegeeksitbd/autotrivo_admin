...
export const getSimpleLineChart = () => ({
  chart: {
    height: 380,
    type: 'line',
    zoom: {
...
export const getZoomableTimeSeriesChart = () => ({
  chart: {
    type: 'area',
    stacked: false,
    height: 360,
...
export const getSyncingCharts2 = () => ({
  chart: {
    height: 200,
    type: 'line',
    id: 'yt',
...
export const getMissingLineChart = () => ({
  chart: {
    height: 380,
    type: 'line',
    zoom: {
...
export const getDashedLineChart = () => ({
  chart: {
    height: 380,
    type: 'line',
    zoom: {
...
export function createDailyTimeSeries(baseval, count, yrange) {
  const series = []
...
export function generateNewPoint(baseval, yrange) {
  const newDate = baseval + 86400000
  const y = Math.floor(Math.random() * (yrange.max - yrange.min + 1)) + yrange.min
  return [newDate, y]
}
export const getRealTimeChartOptions = (baseval) => ({
  chart: {
    height: 350,
    type: 'line',
    animations: {
...
export const getLineChartWithAnnotations = () => ({
  annotations: {
    yaxis: [
      {
        y: 4200,
  ...
  series: [
    {
      ...
      data: [
        ...
        {
          x: new Date('2024-10-10').getTime(),
          y: 4200,
        },
        {
          x: new Date('2024-10-15').getTime(),
          y: 4000,
        },
      ],
    },
